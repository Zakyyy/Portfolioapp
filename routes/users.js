...
//later attach file
router.get('/attachProfilePhotoSec',function(req,res){
  res.render('attachProfilePhotoSec')
});
...
router.post('/ProfilePic',function(req,res){
	upload(req,res,function(err) {
		...
		var conditions={name:req.user.username},update={profilePic:ppsrc};
		User.update(conditions,update,callback);
		function callback(err,numAffected)
		{
			console.log(numAffected);
		}
    if(req.user.numOfWorks=0){
		res.redirect('/projects/createPortfolio');
  }
  else {
    res.redirect('/');
  }
